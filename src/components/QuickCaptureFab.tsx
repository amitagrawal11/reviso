...
import { useRepo, useItems } from '../lib/data-mode';
import { subscribeQuickCapture } from '../lib/quick-capture-bridge';

// The preview pane (LazyMarkdownView + rehype-highlight + highlight.js) is
// only loaded when the user taps the Preview tab — not on Shell mount.
const LazyPreview = lazy(() => import('./QuickCapturePreview'));
...
export function QuickCaptureFab({ bottomOffset = 16, showFab = true }: QuickCaptureFabProps) {
  const repo = useRepo();
  ...
  const openWithParent = useCallback((pid?: string | null) => {
    setParentId(pid ?? null);
    setOpen(true);
  }, []);

  useEffect(() => subscribeQuickCapture(openWithParent), [openWithParent]);
  ...
  useEffect(() => {
    if (open) {
      ...
      void import('./QuickCapturePreview');
    } else {
      setTitle('');
      setContent('');
      setParentId(null);
