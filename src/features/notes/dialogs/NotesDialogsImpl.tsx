    ...
    <Combobox
      ...
      onOptionSubmit={(selected) => {
        if (selected.startsWith('__new__:')) {
          onChange({ kind: 'new', name: selected.slice('__new__:'.length) });
        } else {
          onChange({ kind: 'existing', id: selected });
        }
        setSearch('');
...
function ItemForm({
  ...
  const [icon, setIcon] = useState<string>(emojiSet[0]!);
