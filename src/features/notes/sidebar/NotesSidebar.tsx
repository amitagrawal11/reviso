...
import { prefetchMarkdownViewer } from '@/features/notes/viewer/MarkdownViewer';

// Passed down from AppLayout so note/collection clicks auto-close the mobile drawer.
...
export default function NotesSidebar({ closeMobile }: { closeMobile?: () => void }) {
  const all = useItems();
  ...
  // Folder open/closed state lives at the NotesSidebar level + persists.
  const [openMap, setOpenMap] = useState<Record<string, boolean>>(loadOpenState);
  const isOpen = useCallback(
    (id: string): boolean => openMap[id] ?? false, // default collapsed
    [openMap],
  );
      ...
      <Stack gap="xs" h="100%">
        <Group justify="space-between" px={6}>
          ...
          <Group gap={4}>
            ...
            >
              <ItemIcon icon={FolderPlus} size={16} />
            </ActionIcon>
            <ActionIcon
              size="sm"
              variant="default"
...
function ProfileMenu() {
  ...
  // In demo mode the DemoModeBanner up top handles the sign-in / sign-up CTAs.
      ...
      <Menu.Target>
        ...
        >
          <Group gap="sm" wrap="nowrap" align="center">
            ...
            <ItemIcon icon={ChevronRight} size={16} style={{ flexShrink: 0, opacity: 0.6 }} />
          </Group>
        </UnstyledButton>
      </Menu.Target>
      <Menu.Dropdown>
        ...
        <Menu.Item
          leftSection={<ItemIcon icon={User} size={16} />}
          onClick={() => navigate('/profile')}
        >
          Profile
        </Menu.Item>
        <Menu.Item
          leftSection={<ItemIcon icon={Settings} size={16} />}
          onClick={() => navigate('/settings')}
        >
          Settings
        </Menu.Item>
        <Menu.Divider />
        <Menu.Item color="red" leftSection={<ItemIcon icon={LogOut} size={16} />} onClick={signOut}>
          Sign out
        </Menu.Item>
      </Menu.Dropdown>
    </Menu>
  ...
  return (
    ...
    >
      ...
      <UnstyledButton
        style={{ flex: 1 }}
        onClick={onOpen}
        data-testid="sidebar-folder-link"
        data-href={path(`/c/${item.id}`)}
      >
        <Group gap={6} wrap="nowrap">
    ...
    <div style={{ position: 'relative', overflow: 'hidden', borderRadius: 'var(--radius-sm)' }}>
      ...
      {isTouch && (
        <>
          ...
          >
            <ItemIcon icon={Star} size="md" color="gold" />
          </div>
          <div
            style={{
              position: 'absolute',
          ...
          >
            <ItemIcon icon={Trash2} size="md" style={{ color: 'var(--color-danger)' }} />
          </div>
        </>
      )}
      <Group
        ...
        className={`sidebar-row${isActive ? ' sidebar-row--active' : ''}`}
        wrap="nowrap"
        onMouseEnter={prefetchMarkdownViewer}
      ...
      >
        ...
        >
          <Group gap={6} wrap="nowrap">
            ...
            {item.starred && <ItemIcon icon={Star} size="sm" fill="gold" strokeWidth={0} />}
          </Group>
        </UnstyledButton>
        <div onPointerDown={(e) => e.stopPropagation()}>
          <RowMenu item={item} />
    ...
    return (
      ...
      >
        <Menu.Target>
          ...
          >
            <ItemIcon icon={Ellipsis} size={16} />
          </ActionIcon>
        </Menu.Target>
        <Menu.Dropdown>
          {item.isFolder && (
            <>
              <Menu.Item
                leftSection={<ItemIcon icon={FilePlus} size={16} />}
                onClick={() => openItemDialog({ isFolder: false, parentId: item.id, repo, path })}
              >
                New note inside
              </Menu.Item>
              <Menu.Item
                leftSection={<ItemIcon icon={FolderPlus} size={16} />}
          ...
          <Menu.Item
            leftSection={<ItemIcon icon={Pencil} size={16} />}
          ...
          {!item.isFolder && (
            <Menu.Item
              leftSection={<ItemIcon icon={Star} size={16} />}
          ...
          <Menu.Item
            color="red"
            leftSection={<ItemIcon icon={Trash2} size={16} />}
        ...
        <Stack gap={0} py="xs">
          {item.isFolder && (
            <>
              ...
              >
                <ItemIcon icon={FilePlus} size="md" />
              ...
              >
                <ItemIcon icon={FolderPlus} size="md" />
