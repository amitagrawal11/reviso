...
import { openConfirm } from '@/features/notes/dialogs/NotesDialogs';

const NoteTableOfContents = lazy(() => import('@/features/notes/viewer/NoteTableOfContents'));
...
type RehypePlugins = ComponentProps<typeof MarkdownViewer>['rehypePlugins'];
...
const VIEW_COMPONENTS = { pre: CodeBlockRenderer };
...
export default function NoteViewPage() {
  ...
  // data attribute on <html> so the AppLayout header can display it.
  useEffect(() => {
    if (!isMobile) {
    ...
    >
      {readMode && (
        <Tooltip label="Exit read mode (⌘.)">
          ...
          >
            <ItemIcon icon={BookOpen} size="md" />
          </ActionIcon>
        </Tooltip>
      )}
      {!readMode && (
        <Group justify="space-between" mb="md">
          ...
          <Group gap={6}>
            <Tooltip label={note.starred ? 'Unstar' : 'Star'}>
              ...
              >
                {note.starred ? (
                  <ItemIcon icon={Star} size="md" fill="currentColor" color="gold" />
                ) : (
