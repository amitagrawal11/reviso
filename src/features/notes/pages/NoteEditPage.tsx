...
import type { MarkdownViewer } from '@/features/notes/viewer/MarkdownViewer';
...
const PREVIEW_COMPONENTS = { pre: CodeBlockRenderer };
...
export default function NoteEditPage() {
  ...
  if (!note) return <NoteNotFoundCard kind="note" homePath={path('/')} />;
  ...
  return (
    ...
    >
        ...
        <Group mb="sm" justify="space-between" wrap="wrap" gap="xs">
          ...
          <Group gap="xs" wrap="nowrap">
            <Button
              ...
              size="sm"
              disabled={!dirty}
              leftSection={<ItemIcon icon={Check} size={16} />}
            >
              {isNew ? 'Create' : 'Save'}
            </Button>
      ...
      {/* flex column + overflow hidden so NoteEditor can fill
      ...
      {/* Mobile bottom save bar — fixed above the MobileBottomNavigationBar so it's always
        ...
        <Box className="editor-save-bar">
          ...
          <Group gap="xs">
            ...
            <Button
              ...
              disabled={!dirty}
              onClick={handleSave}
              leftSection={<ItemIcon icon={Check} size={14} />}
            >
              {isNew ? 'Create' : 'Save'}
            </Button>
          </Group>
