...
// If the block is mermaid, render a MermaidDiagramRenderer instead.
export function CodeBlockRenderer(props: React.HTMLAttributes<HTMLPreElement>) {
  ...
  if (isMermaid(props.children) || mermaidByText) {
    return <MermaidDiagramRenderer source={text} />;
  }
  ...
  return (
    <div className="codeblock-wrap" style={{ position: 'relative' }}>
      ...
      <Tooltip label={copied ? 'Copied' : 'Copy code'} withArrow position="left">
        ...
        >
          {copied ? <ItemIcon icon={Check} size="sm" /> : <ItemIcon icon={Copy} size="sm" />}
        </ActionIcon>
      </Tooltip>
    </div>
  );
